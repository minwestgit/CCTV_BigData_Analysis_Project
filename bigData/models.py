...
class Mongo:
    ...
    def Find_HeatMap_Time_Mongo(self,key1,key2, RadioValue):
        ...
        if RadioValue == "all" :
            ...
            "$match" :
                ...
                "TimeStamp" :
                {
                "$gte" : datetime.datetime(int(from_date[0]), int(from_date[1]), int(from_date[2]),int(from_date[3]),int(from_date[4]) ,0),
                "$lte" : datetime.datetime(int(to_date[0]), int(to_date[1]), int(to_date[2]),int(to_date[3]),int(to_date[4]) ,0)
                }
                }
                }
                ,
    ...
    def Find_Person_Mongo(self):
        ...
        PersonData = collection.aggregate(pipline)
        PersonDataList = []
        for line in PersonData:
            PersonDataList.append((list(line.values())))

        PersonDataRealList = []

        for i in PersonDataList:
            PersonDataRealList.append(i[0])

        return PersonDataRealList
...
class MongoAll:
    ...
    collection = database.cols
    key = ""

    # 몽고DB CONNECTION
    def __init__(self):
        self.client = MongoClient('127.0.0.1', 27017);
        self.database = self.client.product
        self.collection = self.database.cols
